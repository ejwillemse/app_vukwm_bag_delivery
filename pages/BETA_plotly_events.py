...
def query_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df["lon_lat"] = df["index"].astype(str)
    df["selected"] = False
    for q in HOVER_INFO:
...
def render_plotly_ui(transformed_df: pd.DataFrame) -> Dict:
    ...
    bill_to_tip_selected = plotly_events(
        bill_to_tip_figure,
        select_event=True,
        key=f"lon_lat_{st.session_state.counter}",
    )
    # with c2:
...
def main():
    data = (
        pd.read_csv(
            "data/test/02_intermediate/geocodes/2022-12-22__03-46-22-429064_Westminster_7_Day_Bag_orders.csv"
        )
        .drop_duplicates(["Site Bk"])
    ...
    st.write(transformed_df.loc[transformed_df["selected"]])
